...
SELECT 
    i.product_id,
    AVG(i.demand) AS avg_demand,
    COUNT(CASE WHEN i.stock_level = 0 THEN 1 END) AS stockout_days
FROM inventory i
...
SELECT 
    ...
    (AVG(demand) * MAX(lead_time)) AS reorder_point
FROM inventory
GROUP BY product_id;
...
SELECT 
    ...
    AVG(demand) AS avg_demand,
    COUNT(CASE WHEN stock_level = 0 THEN 1 END) AS stockout_days
FROM inventory
GROUP BY product_id;